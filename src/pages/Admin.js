...
const BASE_URL = process.env.REACT_APP_SERVER_URL ||  "http://127.0.0.1:7500"
...
function Admin() {
  ...
  const fetchUsers = (userId) => {
    axios
      .get(BASE_URL + "/crm/api/users/" + userId, {
        headers: {
  ...
  const updateUserDetail = () => {
    ...
    axios
      .put(
        BASE_URL + "/crm/api/users/" + userDetail.userId,
        data,
        {
          headers: {
            "x-access-token": localStorage.getItem("token"),
  ...
  const changeUserDetail = (e) => {
    ...
    else if (e.target.name === "type") userDetail.userType = e.target.value;
        ...
        <div className="container">
          <div>
            ...
            <p className="text-muted text-center">
              Take a quick look at you admin stats below.
            </p>
